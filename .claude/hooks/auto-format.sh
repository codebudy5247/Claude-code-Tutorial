...
# Debug line
echo "[auto-format] triggered for: $FILE_PATH" >> .claude/hooks/debug.log

# Detect file type and format accordingly
case "$FILE_PATH" in
  *.js|*.jsx|*.ts|*.tsx)
    PRETTIER=$(command -v prettier || echo "./node_modules/.bin/prettier")
    if [ -f "$PRETTIER" ] || command -v prettier &> /dev/null; then
